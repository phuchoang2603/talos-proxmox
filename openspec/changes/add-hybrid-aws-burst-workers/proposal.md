...
- Use one `terraform/cluster/` root with Proxmox and AWS modules and separate per-environment MinIO state keys. Manage the GitHub OIDC CI role in an independent `terraform/identity/` root.
...
- One OpenTofu cluster root with Proxmox and AWS modules, per-environment MinIO keys, Talos secrets, API/network routing, and AWS scheduling; the independent identity root manages the CI IAM role.
- GitHub Actions PR lint/main applies, existing Doppler and main-only GitHub Environments, a dedicated GitHub OIDC CI role, and a future separate autoscaler identity.
