...
Configs: `dev`, `prod`, and `argocd` (same names as git env folders and GitHub Environments). Shared keys are in all three.
...
CI needs a read/write service token as GitHub secret `DOPPLER_TOKEN` per Environment (terraform apply and cluster credential writes). The argocd config also stores **`DOPPLER_READ_TOKEN`**: a separate project read token used by `bootstrap-argocd.sh` to fetch dev/prod kubeconfigs.
