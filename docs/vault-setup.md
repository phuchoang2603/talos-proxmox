...
Cluster IPs used by Terraform (`vip`, `lb_range`) live in git: `terraform-provision/env/{dev,prod}/network.json`. Gateway LAN IPs are in `apps/manifests/env/{dev,prod}/`. Do not store them in Vault.
